  ...
  return (
    <>
      <PageHeader
        ...
        description="Platform spend and platform ROAS next to GA4 sessions and engagement. Platform ROAS is diagnostic only: each platform claims conversions under its own attribution, so it is never summed across platforms."
        right={
          <Badge tone={matchRate > 0.9 ? "good" : "warn"}>
            UTM match rate {fmtPct(matchRate)}
          </Badge>
        }
      />

      <Card subtitle="Last 28 days, sorted by spend. GA4 columns join via the campaign crosswalk where UTM tagging allows.">
          ...
          <thead>
            <tr className="border-b border-slate-800 text-left text-xs uppercase tracking-wide text-slate-500">
              ...
              <th className="pb-2 pr-4 text-right font-medium">
                ROAS <Badge tone="warn">platform reported</Badge>
              </th>
            ...
            {campaigns.map((c) => (
              <tr key={`${c.platform}-${c.name}`} className="text-slate-300">
                ...
                <td className="py-2.5 pr-4 text-right tabular-nums">
                  {c.ga4Sessions !== null ? (
                    fmtNumCompact(c.ga4Sessions)
                  ) : (
                    <Badge tone="warn">no UTM match</Badge>
                  )}
                </td>
